...
- `Superhuman Waitlist Landing Page Concept.md` → **hole-fill (research).** Rich external research capture on Superhuman's high-friction waitlist funnel (request-access → qualification survey → referral/bypass loop → 30-min concierge onboarding → $30/mo prepay). Relevant to Exo Launch / future launch gate + lead-magnet backlog. Added full frontmatter (type:research) + triage note + backlog items. Below wiki-create threshold — kept as capture/reference.
- No new wiki pages → index unchanged. No SSOT promote (ideation/research, not locked decisions). No Reservoir task spawned (all backlog-gated, none actionable today).
- Queue remaining: **0** new loose/untriaged root notes (only excluded system/test junk at root).
